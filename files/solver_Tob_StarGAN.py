from files.model_M_OctConv import Generator,Discriminator
...
class Solver(object):
    ...
    def __init__(self, celeba_loader, rafd_loader, config):
        ...
        self.lr_update_step = config.lr_update_step; self.alpha1 = config.alpha1; self.alpha2 = config.alpha2; print("###########{}-{}##########".format(self.alpha1, self.alpha2))
    ...
    def build_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD']:
            self.G = Generator(self.g_conv_dim, self.c_dim, self.g_repeat_num, self.alpha1, self.alpha2)
            self.D = Discriminator(self.image_size, self.d_conv_dim, self.c_dim, self.d_repeat_num, 3, self.alpha1, self.alpha2) 
        elif self.dataset in ['Both']:
            self.G = Generator(self.g_conv_dim, self.c_dim+self.c2_dim+2, self.g_repeat_num, self.alpha1, self.alpha2)   # 2 for mask vector.
            self.D = Discriminator(self.image_size, self.d_conv_dim, self.c_dim+self.c2_dim, self.d_repeat_num, 3, self.alpha1, self.alpha2)
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            out_src_real, out_cls = self.D(x_real)
            ...
            d_loss_real = (torch.mean((out_src_real - torch.mean(out_src_fake) - 1) ** 2))/2
            d_loss_fake = (torch.mean((out_src_fake - torch.mean(out_src_real) + 1) ** 2))/2
            ...
            if (i+1) % self.n_critic == 0:
                ...
                out_src_fake, out_cls = self.D(x_fake)

                #g_loss_fake = - torch.mean(out_src)
                g_loss_fake = (torch.mean((out_src_real - torch.mean(out_src_fake) + 1) ** 2) 
                            + torch.mean((out_src_fake - torch.mean(out_src_real) - 1) ** 2))/2
    ...
    def test_multi(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(self.celeba_loader):
                ...
                # Translate images.
                print(c_celeba_list);print(mask_celeba);print(zero_celeba);x_fake_list = [x_real]
